...
const defaultProducts = [
  ...
  { id: 'p14', name: 'Mega Set de Arte 208 Pzas', price: 55000, category: 'niños', image: 'img/productos/arte.jpeg' },
  { id: 'p15', name: 'Tablero Didáctico Kuromi', price: 22000, category: 'niños', image: 'img/productos/tablero.jpeg' },
  { id: 'p16', name: 'Lonchera Encanto', price: 10000, category: 'niños', image: 'img/productos/lonchera.jpeg' },
  { id: 'p17', name: 'Bolso Infantil 3 en 1', price: 50000, category: 'niños', image: 'img/productos/bolso.jpeg' },
...
const defaultUsers = [
  ...
  { email: 'dueño@saemi.com', name: 'Dueño', address: 'Tienda principal', phone: '3000000002', role: 'dueño' },
...
function renderProducts() {
  ...
  if (!filtered.length) {
    productGrid.innerHTML = '<p class="empty-message">No hay productos en esta categoría.</p>';
    return;
  }
...
function renderOrders() {
  ...
  if (!orders.length) {
    ordersPanelContent.innerHTML = '<p class="empty-message">No hay pedidos todavía.</p>';
    return;
  }
...
function applyRoleUI() {
  ...
  managerPanel.classList.toggle('hidden', role !== 'dueño' && role !== 'admin');
...
function renderUserUI() {
  if (!currentUser) {
    welcomeMessage.textContent = 'Bienvenido, crea una cuenta o inicia sesión para hacer pedidos.';
...
function loginWithEmail(email) {
  ...
  if (!user) {
    showToast('Correo no registrado. Regístrate primero.');
    return;
  }
  currentUser = user;
  saveAll();
...
function checkout() {
  if (!currentUser) {
    showToast('Debes iniciar sesión o registrarte para finalizar la compra.');
    openAuthModal();
    return;
  }
  ...
  renderOrders();
  closeCart();
  showToast('Compra realizada. Tu pedido se enviará a ' + currentUser.address + '.');
}
